---
title: "Mortal Shell 2 Shells — Locations, Abilities & Best Shells"
description: "Find all nine Mortal Shell 2 Shells, including locations, unlock requirements, practical playstyles, Shell and weapon pairings, and the permanent Shell routes."
category: combat-exploration
slug: mortal-shell-ii/shells
status: needs-verification
featured: false
quickAnswer: "Mortal Shell II has nine Shells when the prologue Harros is counted: eight permanent unlocks plus the opening Shell. Find each Shell at its region landmark, complete its first Memory or unlock sequence, then use the Shellkeeper and Glimpses for Bonding; Shell-location reveals use Gloom after Balance Patch 1."
related:
...
For the side-quest layer that used to be mixed into this hub, see the dedicated [NPC questlines guide](/mortal-shell-ii/npc-questlines/). It keeps Gorf, Hilga, Gragu, Baghead, Egon, Genessa, and the explicitly incomplete Vlas evidence in one place without changing the Shell unlock routes below.

## All Shell locations and unlock conditions (Item 11 coverage retained)
...
## Shell and weapon pairings

The current evidence supports one concrete starting pairing: **Tiel with Axe & Dagger**. The [Axe & Dagger guide](/mortal-shell-ii/axe-dagger/) describes Tiel as a practical pairing to test, while the [Tiel route](/mortal-shell-ii/tiel/) supports the mobile, evasive playstyle that makes the combination a reasonable early experiment.

Treat that as route and playstyle advice, not an exclusive or mathematically best build. The other Shells can use the shared weapon roster, but this checkout does not contain controlled testing for a complete Shell-to-weapon matrix. Use the [Weapons guide](/mortal-shell-ii/weapons/) to choose by route, weapon identity, and current patch context.

## Related routes

Use these routes to turn Shell collection into a practical run plan:

- [Fainweald Shell route](/mortal-shell-ii/routes/world-progression/#map-route-fainweald-shells/) — the current mapped route for the permanent Shell objectives it covers.
- [World and Progression route](/mortal-shell-ii/routes/world-progression/) — the broader Prologue → Fainweald → Mammon → Unfound Path sequence.
- [Mortal Shell II map](/mortal-shell-ii/map/) — use the map when a Shell or nearby route marker is available; do not treat an unmapped landmark as a verified coordinate.
- [NPC questlines](/mortal-shell-ii/npc-questlines/) — side-quest timing, handoffs, and current evidence limits.
